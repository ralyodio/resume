...
test('cli can approve queued jobs and dry-run external apply reports auto-apply status',()=>{ const dir=fs.mkdtempSync(path.join(os.tmpdir(),'jobs-cli-')); const s=new JobStore(dir); s.enqueue({id:'j1',source:'web3-career',title:'AI Engineer',company:'Co',status:'queued',sourceUrl:'https://example.com/job',applyUrl:'https://example.com/apply'}); let r=spawnSync(process.execPath,['src/cli.cjs','jobs','approve','--id','j1','--store',dir],{cwd:__dirname+'/..',encoding:'utf8'}); assert.equal(r.status,0,r.stderr); r=spawnSync(process.execPath,['src/cli.cjs','jobs','apply','--approved','--store',dir],{cwd:__dirname+'/..',encoding:'utf8'}); assert.equal(r.status,0,r.stderr); assert.match(r.stdout,/auto-apply\tprepared\tj1\tunknown/); assert.equal(new JobStore(dir).get('j1').status,'approved'); });
...
test('jobs queue respects scorer decisions and never queues skip decisions',()=>{
  ...
  s.upsert({id:'q1',source:'web3-career',title:'AI Engineer',company:'Co',location:'Remote',status:'scored',score:90,decision:'queue-for-review'});
  ...
  s.upsert({id:'skip1',source:'web3-career',title:'AI Engineer',company:'Co',location:'Remote',status:'scored',score:99,decision:'skip',riskFlags:['scam language']});
  s.upsert({id:'save1',source:'web3-career',title:'AI Engineer',company:'Co',location:'Remote',status:'scored',score:75,decision:'save-only'});
