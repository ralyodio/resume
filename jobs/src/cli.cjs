#!/usr/bin/env node
// Load .env from repo root (two levels up: jobs/src -> jobs -> repo root)
(function loadEnv() {
...
function isKnownBlockedUrl(url){
  if (!url) return false;
  if (/jobicy\.com/i.test(url)) return true; // Cloudflare blocks Browserbase IPs
  if (/\/apply\/applyManually|\/apply\/autofillWithResume/i.test(url)) return true; // Workday flow has no top-level submit
  return false;
}
function queueScored(store, min){ const repoRoot=path.resolve(__dirname,'..','..'); let n=0; let blocked=0; const queueDecisions=new Set(['queue-for-review','auto-apply-eligible']); for(const job of store.all().filter(j=>(j.score||0)>=min && ['new','scored'].includes(j.status) && queueDecisions.has(j.decision))){ const url = job.applyUrl||job.sourceUrl||''; if (isKnownBlockedUrl(url)) { store.skip(job.id,'known-blocked-url'); blocked++; continue; } const resumePath=selectResume(job); const coverPdfPath=process.env.COVER_PDF || path.join(repoRoot,'anthony.ettinger.cover4.pdf'); const coverLetter=generateCoverLetter(job); store.upsert({...job,status:'queued',resumePath,coverPdfPath,coverLetter},'queue'); n++; console.log(`queued\t${job.score}\t${job.title}\t${job.company}`); } if (blocked) console.error(`skipped ${blocked} known-blocked URLs (jobicy/workday-applyManually)`); return n; }
